...
-- Default penalty rules (admin can update via UI)
INSERT INTO penalty_rules (id, type, rate, is_percent, description, is_active)
VALUES
  (uuid_generate_v4(), 'missed_contribution', 100.00, false, 'Fixed KES 100 penalty for missed monthly contribution', true),
  (uuid_generate_v4(), 'late_repayment', 5.00, true, '5% of outstanding amount for late loan repayment', true),
  (uuid_generate_v4(), 'rule_violation', 500.00, false, 'Fixed KES 500 for SACCO rule violations', true),
  (uuid_generate_v4(), 'other', 200.00, false, 'Default penalty for other violations', true)
ON CONFLICT DO NOTHING;

-- Default loan products
INSERT INTO loan_products (id, name, description, interest_rate, interest_method, min_amount, max_amount, min_term_months, max_term_months, max_multiplier, guarantors_required, processing_fee_pct, is_active)
VALUES
  (uuid_generate_v4(), 'Emergency Loan', 'Short-term emergency financial assistance', 12.00, 'reducing', 5000, 50000, 1, 6, 2.0, 1, 0.50, true),
  (uuid_generate_v4(), 'Development Loan', 'Medium-term development and business loans', 14.00, 'reducing', 10000, 500000, 6, 36, 4.0, 2, 1.00, true),
  (uuid_generate_v4(), 'School Fees Loan', 'Education financing for members and dependants', 10.00, 'flat', 5000, 100000, 3, 12, 3.0, 1, 0.50, true),
  (uuid_generate_v4(), 'Super Saver Loan', 'Premium loan for long-serving members with high savings', 10.00, 'reducing', 50000, 2000000, 12, 60, 6.0, 3, 1.00, true)
ON CONFLICT DO NOTHING;

-- Default SACCO settings
INSERT INTO sacco_settings (key, value, description)
VALUES
  ('sacco_name', 'UmojaSACCO', 'Full SACCO name'),
  ('contribution_deadline_day', '5', 'Day of month contributions are due'),
  ('min_shares', '500', 'Minimum monthly shares contribution (KES)'),
  ('min_savings', '1000', 'Minimum monthly savings contribution (KES)'),
  ('welfare_contribution', '200', 'Monthly welfare contribution (KES)'),
  ('dividend_rate', '8.00', 'Annual dividend rate (%)'),
  ('max_loan_multiplier', '4.0', 'Maximum loan as multiple of savings')
ON CONFLICT DO NOTHING;
